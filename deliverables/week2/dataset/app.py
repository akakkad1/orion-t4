...
# ── Snapshot export (data note deliverable) ───────────────────────────────────
def _vessels_to_jsonable(vessels):
    """st.session_state.vessels stores track points as tuples; JSON needs
    lists. This produces a plain-dict copy safe for json.dumps / downstream
    cleaning in snapshot_vessels.py, without mutating session state."""
    out = {}
    for vid, v in vessels.items():
        v2 = dict(v)
        v2["track"] = [list(pt) for pt in v.get("track", [])]
        out[vid] = v2
    return out
...
with st.sidebar:
    ...
    # ── Snapshot export ───────────────────────────────────────────────────────
    # Exports the current in-memory vessels dict as JSON, for use with
    # snapshot_vessels.py to produce a cleaned, saved CSV pair. This is a
    # point-in-time capture of the live feed, not a persistent dataset — see
    # the accompanying data note.
    st.markdown('<div class="section-label">💾 Snapshot Export</div>', unsafe_allow_html=True)
    n_snapshot = len(st.session_state.vessels)
    st.caption(f"{n_snapshot} vessel record(s) currently in memory.")

    if n_snapshot == 0:
        st.button("⬇ Download vessels.json", use_container_width=True, disabled=True,
                   key="snapshot_disabled", help="No vessels loaded yet — fetch or wait for AIS pings first.")
    else:
        snapshot_payload = json.dumps(_vessels_to_jsonable(st.session_state.vessels), indent=2)
        snapshot_stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        st.download_button(
            "⬇ Download vessels.json",
            data=snapshot_payload,
            file_name=f"vessels_{snapshot_stamp}.json",
            mime="application/json",
            use_container_width=True,
            key="snapshot_download",
            help="Run snapshot_vessels.py against this file to get cleaned, saved CSVs.",
        )

...
os.makedirs(f"data/{RUN_ID}/graphs", exist_ok=True) # Store generated HTML map files
...
def pull_opensky(airport: str) -> pd.DataFrame:
    ...
    df.to_csv(f"data/{RUN_ID}/raw/opensky_{airport}_raw.csv", index=False)
    # Print confirmation with file path and row/column counts (shape)
    print(f"  Saved raw  → data/{RUN_ID}/raw/opensky_{airport}_raw.csv  {df.shape}")
...
def clean_opensky(df: pd.DataFrame, airport: str) -> pd.DataFrame:
    ...
    df.to_csv(f"data/{RUN_ID}/clean/opensky_{airport}_clean.csv", index=False)
    # Print confirmation with file path and row/column counts
    print(f"  Saved clean → data/{RUN_ID}/clean/opensky_{airport}_clean.csv  {df.shape}")
...
def graph_opensky(df: pd.DataFrame, airport: str):
    ...
    path = f"data/{RUN_ID}/graphs/opensky_{airport}_map.html"
    m.save(path)
...
def pull_ais(bbox: list) -> pd.DataFrame:
    ...
    df.to_csv("data/{RUN_ID}/raw/ais_raw.csv", index=False)
    print(f"  Saved raw  → data/{RUN_ID}/raw/ais_raw.csv  {df.shape}")
    return df
...
def clean_ais(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.to_csv("data/{RUN_ID}/clean/ais_clean.csv", index=False)
    
    # Print confirmation with file path and data shape (rows, columns)
    print(f"  Saved clean → data/{RUN_ID}/clean/ais_clean.csv  {df.shape}")
...
def graph_ais(df: pd.DataFrame):
    ...
    m.save("data/{RUN_ID}/graphs/ais_map.html")
    
    # Print confirmation with file path and number of vessels mapped
    print(f"  Saved map  → data/{RUN_ID}/graphs/ais_map.html  ({len(df)} vessels)")

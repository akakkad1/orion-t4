...
os.makedirs("data/raw",    exist_ok=True)    # Store raw API responses
os.makedirs("data/clean",  exist_ok=True)    # Store cleaned/processed data
os.makedirs("data/graphs", exist_ok=True)    # Store generated HTML map files
...
def pull_opensky(airport: str) -> pd.DataFrame:
    ...
    df.to_csv(f"data/raw/opensky_{airport}_raw.csv", index=False)
    # Print confirmation with file path and row/column counts (shape)
    print(f"  Saved raw  → data/raw/opensky_{airport}_raw.csv  {df.shape}")
...
def clean_opensky(df: pd.DataFrame, airport: str) -> pd.DataFrame:
    ...
    df.to_csv(f"data/clean/opensky_{airport}_clean.csv", index=False)
    # Print confirmation with file path and row/column counts
    print(f"  Saved clean → data/clean/opensky_{airport}_clean.csv  {df.shape}")
...
def graph_opensky(df: pd.DataFrame, airport: str):
    ...
    path = f"data/graphs/opensky_{airport}_map.html"
    m.save(path)
...
def pull_ais(bbox: list) -> pd.DataFrame:
    ...
    print(f"  Saved raw  → data/raw/ais_raw.csv  {df.shape}")
    return df
...
def clean_ais(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.to_csv("data/clean/ais_clean.csv", index=False)
    
    # Print confirmation with file path and data shape (rows, columns)
    print(f"  Saved clean → data/clean/ais_clean.csv  {df.shape}")
...
def graph_ais(df: pd.DataFrame):
    ...
    m.save("data/graphs/ais_map.html")
    
    # Print confirmation with file path and number of vessels mapped
    print(f"  Saved map  → data/graphs/ais_map.html  ({len(df)} vessels)")
